...
- **`app.py`** — a [FastAPI](https://fastapi.tiangolo.com/) application served
  by `uvicorn`. Two routes, plus the interactive docs FastAPI generates for
  free at `/docs`. Add dependencies to the `uv pip install` line in the
  Dockerfile as it grows.
